...
# Define static text about Kashish
static_text = """
...
# Calculate cosine similarity between user message and static text
def calculate_similarity(user_message, text_content):
    user_tokens = word_tokenize(user_message.lower())
    text_tokens = word_tokenize(text_content.lower())
    user_sentence = ' '.join(user_tokens)
...
def reply(user_message, option):
    if option == "Chat Kashish":
        ...
        # Configure GenAI model for conversation
        generation_config = {
            "temperature": 0.7,
            "top_p": 1,
            "top_k": 1,
        ...
        # Start chat with user message and static text
        convo = model.start_chat(history=[
            {
                "role": "user",
                "parts": [user_message]
        ...
        # Fallback if option is not recognized
